...
INVERTEXTO_TOKEN = '25200|KwPHTiSkLrFOaAVkbh6IC8WB1l1MDqKU'
...
def consulta(cep_input):
    ...
    if len(cep_formatado) != 8:
        return jsonify({"erro": "CEP invalido! Deve conter 8 dígitos"}), 400
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            cursor.close()
            conn.close()
            return jsonify({"error": "Erro ao encontrar API externa"}), response.status_code
